...
class MainController:
    ...
    def perform_action(self):
        ...
        rospy.loginfo_throttle(1, "MainController Action#[%04d] State: %s. VictimFound: %s . GoalFound: %s" %  \
            (self.action_seq, str(self.state), self.victim_pose is not None, self.goalzone_pose is not None))
        ...
        if self.state is State.INIT:
            ...
            if self.victim_pose is None:
                rospy.logerr("Trying to fetch victim when they havent been detected yet.")
            if self.goto_pose(self.victim_pose):
                self.state = State.DELIVER_TREASURE
                rospy.loginfo("Reached victim! Heading to delivery now")
        
        elif self.state is State.DELIVER_TREASURE:
            if self.goto_pose(self.goalzone_pose):
                self.state = State.DONE
                rospy.loginfo("Delivered victim! All done now!")
        ...
        while not rospy.is_shutdown():
            try:
                ...
                if self.state is State.SEARCH_OBJECTIVES and self.mapping_is_complete():
                    ...
                    if self.victim_pose is None: feedback.append("Victim not located")
    ...
    def mapping_is_complete(self):
        return self.victim_pose is not None and self.goalzone_pose is not None
    ...
    def goto_pose(self, pose):
        ...
        if not waiting:
            ...
            rospy.loginfo("Victim seeking result" + str(client.get_result()))
            return True
    ...
    def ar_callback(self, msg):
        ...
        for marker in msg.markers:
            if marker.id is 0:
                if self.victim_pose is None:
                    rospy.loginfo("Located victim")
                self.victim_pose = marker.pose
                self.victim_pose.header = Header(frame_id="map")
                self.victim_pose.header.stamp = rospy.Time.now()
    ...
    def visualize_info(self):
        ...
        if self.victim_pose is not None:
            rospy.logdebug("Visualizing Victim")
            marker = Marker(
                type=Marker.SPHERE,
                action=Marker.ADD,
                id=MarkerIds.VICTIM,
                lifetime=rospy.Duration(secs=0),
                scale=Vector3(0.1, 0.1, 0.1),
                header=self.victim_pose.header,
                color=ColorRGBA(0, 1.0, 0, 1.0),
                pose=self.victim_pose.pose,
                points=[self.victim_pose.pose.position],
                text="Victim F"
            )
            self.visualizations_pub.publish(marker)
